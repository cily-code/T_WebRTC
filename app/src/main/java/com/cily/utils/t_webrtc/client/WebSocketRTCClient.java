...
import com.cily.utils.t_webrtc.utils.JsonUtils;
...
public class WebSocketRTCClient implements AppRTCClient, WebSocketChannelEvents {
    ...
    private void connectToRoomInternal() {
        String connectionUrl = ConnUtils.getConnectionUrl(connectionParameters);
        L.d(TAG, "Connect to room: " + connectionUrl);
        roomState = ConnectionState.NEW;
    ...
    private void disconnectFromRoomInternal() {
        L.d(TAG, "Disconnect. Room state: " + roomState);
        if (roomState == ConnectionState.CONNECTED) {
            L.d(TAG, "Closing room.");
    ...
    private void signalingParametersReady(final SignalingParameters signalingParameters) {
        ...
        messageUrl = ConnUtils.getMessageUrl(connectionParameters, signalingParameters);
        leaveUrl = ConnUtils.getLeaveUrl(connectionParameters, signalingParameters);
        L.d(TAG, "Message URL: " + messageUrl);
        L.d(TAG, "Leave URL: " + leaveUrl);
...
//        wsClient.send("发送数据");
    ...
    public void sendOfferSdp(final SessionDescription sdp) {
        handler.post(new Runnable() {
            ...
            public void run() {
                ...
                JsonUtils.jsonPut(json, "type", "offer");
    ...
    public void sendAnswerSdp(final SessionDescription sdp) {
        handler.post(new Runnable() {
            ...
            public void run() {
                if (connectionParameters.loopback) {
                    L.e(TAG, "Sending answer in loopback mode.");
                    return;
                }
                JSONObject json = new JSONObject();
                JsonUtils.jsonPut(json, "sdp", sdp.description);
                JsonUtils.jsonPut(json, "type", "answer");
...
//                    sendPostMessage(MessageType.MESSAGE, messageUrl, json.toString());
                    sendPostMessage(MessageType.MESSAGE, messageUrl, JsonUtils.toJsonStr(map));
                    if (connectionParameters.loopback) {
...
//                    wsClient.send(json.toString());
                    wsClient.send(JsonUtils.toJsonStr(map));
                }
            }
        });
    }
    ...
    public void sendLocalIceCandidateRemovals(final IceCandidate[] candidates) {
        handler.post(new Runnable() {
            ...
            public void run() {
                JSONObject json = new JSONObject();
                JsonUtils.jsonPut(json, "type", "remove-candidates");
                JSONArray jsonArray = new JSONArray();
                for (final IceCandidate candidate : candidates) {
                    jsonArray.put(JsonUtils.toJsonCandidate(candidate));
                }
                JsonUtils.jsonPut(json, "candidates", jsonArray);
                if (initiator) {
                    // Call initiator sends ice candidates to GAE server.
                    if (roomState != ConnectionState.CONNECTED) {
    ...
    public void onWebSocketMessage(final String msg) {
        if (wsClient.getState() != WebSocketChannelClient.WebSocketConnectionState.REGISTERED) {
            L.e(TAG, "Got WebSocket message in non registered state.");
            return;
        }
        try {
            ...
            if (msgText.length() > 0) {
                ...
                if (type.equals("candidate")) {
                    events.onRemoteIceCandidate(JsonUtils.toJavaCandidate(json));
                } else if (type.equals("remove-candidates")) {
                    JSONArray candidateArray = json.getJSONArray("candidates");
                    IceCandidate[] candidates = new IceCandidate[candidateArray.length()];
                    for (int i = 0; i < candidateArray.length(); ++i) {
                        candidates[i] = JsonUtils.toJavaCandidate(candidateArray.getJSONObject(i));
                    }
                    events.onRemoteIceCandidatesRemoved(candidates);
...
//                                String result = roomJson.getString("result");
                                String result = JsonUtils.getStrFromJson(JsonUtils.toJson(response), "result");
