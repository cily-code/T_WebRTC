...
import com.cily.utils.t_webrtc.impl.TCPChannelEvents;
...
public class TCPChannelClient {
    ...
    private abstract class TCPSocket extends Thread {
        ...
        @Override
        public void run() {
            L.d(TAG, "Listening thread started...");

            // Receive connection to temporary variable first, so we don't block.
            Socket tempSocket = connect();
            BufferedReader in;

            L.d(TAG, "TCP connection established.");

            synchronized (rawSocketLock) {
                if (rawSocket != null) {
                    L.e(TAG, "Socket already existed and will be replaced.");
                }
            ...
            L.v(TAG, "Execute onTCPConnected");
            executor.execute(new Runnable() {
                @Override
                public void run() {
                    L.v(TAG, "Run onTCPConnected");
            ...
            while (true) {
                ...
                executor.execute(new Runnable() {
                    @Override
                    public void run() {
                        L.v(TAG, "Receive: " + message);
                        eventListener.onTCPMessage(message);
                    }
                });
            }

            L.d(TAG, "Receiving thread exiting...");
    ...
    private class TCPSocketServer extends TCPSocket {
        ...
        public Socket connect() {
            L.d(TAG, "Listening on [" + address.getHostAddress() + "]:" + Integer.toString(port));
            ...
            synchronized (rawSocketLock) {
                if (serverSocket != null) {
                    L.e(TAG, "Server rawSocket was already listening and new will be opened.");
                }
    ...
    private class TCPSocketClient extends TCPSocket {
        ...
        @Override
        public Socket connect() {
            L.d(TAG, "Connecting to [" + address.getHostAddress() + "]:" + Integer.toString(port));
